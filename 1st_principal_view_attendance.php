...
$subject_query = "SELECT DISTINCT subject_name FROM first_attendance ORDER BY subject_name";
$subject_result = $conn->query($subject_query);

$year_query = "SELECT DISTINCT YEAR(date) as year FROM first_attendance ORDER BY year DESC";
...
$base_query = "FROM first_attendance 
               JOIN first_students ON first_attendance.roll_number = first_students.roll_number 
               WHERE 1=1";

if ($search_name) {
    $base_query .= " AND first_students.name LIKE '%$search_name%'";
}
if ($search_roll) {
    $base_query .= " AND first_students.roll_number LIKE '%$search_roll%'";
}
if ($search_subject) {
    $base_query .= " AND first_attendance.subject_name = '$search_subject'";
}
if ($search_date) {
    $base_query .= " AND first_attendance.date = '$search_date'";
}
if ($search_status) {
    $base_query .= " AND first_attendance.status = '$search_status'";
}
if ($search_week) {
    $base_query .= " AND WEEK(first_attendance.date, 1) = WEEK('$search_week', 1) AND YEAR(first_attendance.date) = YEAR('$search_week')";
}
if ($search_month && $search_year) {
    $base_query .= " AND MONTH(first_attendance.date) = '$search_month' AND YEAR(first_attendance.date) = '$search_year'";
}

$first_attendance_query = "SELECT first_students.name AS first_student_name, first_students.roll_number, 
                            first_attendance.subject_name, first_attendance.status, first_attendance.date 
                     $base_query 
                     ORDER BY first_attendance.date DESC, first_students.name ASC
                     LIMIT $limit OFFSET $offset";

$first_attendance_result = $conn->query($first_attendance_query);
if (!$first_attendance_result) {
        ...
        <div class="card-body">
            <form method="GET" class="row g-3">
                ...
                <div class="col-md-6 col-lg-3 d-flex align-items-end">
                    ...
                    <a href="1st_principal_view_attendance.php" class="btn btn-outline-secondary">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-hover">
                    ...
                    <tbody>
                        <?php if ($first_attendance_result->num_rows > 0): ?>
                            <?php while ($row = $first_attendance_result->fetch_assoc()): 
                                $date = new DateTime($row['date']);
                                $dayName = $date->format('l');
                                $formattedDate = $date->format('M j, Y');
                            ?>
                                <tr>
                                    <td><?= htmlspecialchars($row['first_student_name']); ?></td>
