...
if ($_SERVER["REQUEST_METHOD"] != "POST") {
    header("Location: 1st_mark_attendance.php");
    exit();
}
...
foreach ($status as $roll_number => $first_attendance_status) {
    try {
        // 4.1 Fetch student name
        $studentQuery = "SELECT name FROM first_students WHERE roll_number = ?";
        ...
        $insertQuery = "INSERT INTO first_attendance 
...
header("Location: 1st_attendance_success.php");
exit();
?>