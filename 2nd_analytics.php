...
if ($filter === "today") {
    $filter_conditions[] = "second_attendance.date = CURDATE()";
} elseif ($filter === "yesterday") {
    $filter_conditions[] = "second_attendance.date = DATE_SUB(CURDATE(), INTERVAL 1 DAY)";
} elseif ($filter === "week") {
    $filter_conditions[] = "second_attendance.date >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)";
} elseif ($filter === "month") {
    $filter_conditions[] = "second_attendance.date >= DATE_SUB(CURDATE(), INTERVAL 1 MONTH)";
}

// Apply custom date range filter
if (!empty($start_date) && !empty($end_date)) {
    $filter_conditions[] = "second_attendance.date BETWEEN '" . $conn->real_escape_string($start_date) . "' AND '" . $conn->real_escape_string($end_date) . "'";
}

// Apply subject filter
if (!empty($subject_filter) && $subject_filter !== "all") {
    $filter_conditions[] = "second_attendance.subject_name = '" . $conn->real_escape_string($subject_filter) . "'";
}

// Apply student filter
if (!empty($student_filter) && $student_filter !== "all") {
    $filter_conditions[] = "second_attendance.roll_number = '" . $conn->real_escape_string($student_filter) . "'";
}
...
// Total present count
$total_present_query = "SELECT COUNT(*) AS count FROM second_attendance WHERE status='Present' " . (!empty($where_clause) ? " AND " . implode(" AND ", $filter_conditions) : "");
$total_present = $conn->query($total_present_query)->fetch_assoc()['count'];

// Total attendance records
$total_records_query = "SELECT COUNT(*) AS count FROM second_attendance " . (!empty($where_clause) ? $where_clause : "");
...
$subject_result = $conn->query("SELECT DISTINCT subject_name FROM second_attendance ORDER BY subject_name");

// Fetch all students for dropdown
$student_result = $conn->query("SELECT DISTINCT roll_number, student_name FROM second_attendance ORDER BY student_name");
...
<body>
    ...
    <div class="dashboard-header d-block d-md-none">
        <div class="d-flex justify-content-between align-items-center">
            ...
            <a href="admin_dashboard.php" class="btn btn-light btn-sm rounded-pill">
        ...
        <div class="card animated-card">
            ...
            <div class="card-body">
                <form method="GET" class="row g-2 filter-form">
                    ...
                    <div class="col-6 col-md-3 mt-2">
                        <a href="analytics.php" class="btn btn-outline-secondary btn-sm w-100">
