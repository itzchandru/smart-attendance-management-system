...
if ($_SERVER["REQUEST_METHOD"] != "POST") {
    header("Location: 4th_mark_attendance.php");
    exit();
}
...
$year = 4; // For 2nd year students
$faculty_id = $_SESSION['faculty_id'];
$faculty_name = $_SESSION['faculty_name'] ?? 'Faculty';

// 4. PROCESS EACH STUDENT
foreach ($status as $roll_number => $fourth_attendance_status) {
    try {
        // 4.1 Fetch student name
        $studentQuery = "SELECT name FROM fourth_students WHERE roll_number = ?";
        ...
        $insertQuery = "INSERT INTO fourth_attendance 
...
header("Location: 4th_attendance_success.php");
exit();
?>