...
$subject_query = "SELECT DISTINCT subject_name FROM second_attendance ORDER BY subject_name";
$subject_result = $conn->query($subject_query);

$year_query = "SELECT DISTINCT YEAR(date) as year FROM second_attendance ORDER BY year DESC";
$year_result = $conn->query($year_query);

$month_query = "SELECT DISTINCT MONTH(date) as month, MONTHNAME(date) as month_name 
                FROM second_attendance 
                ORDER BY month";
...
$base_query = "FROM second_attendance 
               JOIN second_students ON second_attendance.roll_number = second_students.roll_number 
               WHERE 1=1";

if ($search_name) {
    $base_query .= " AND second_students.name LIKE '%$search_name%'";
}
if ($search_roll) {
    $base_query .= " AND second_students.roll_number LIKE '%$search_roll%'";
}
if ($search_subject) {
    $base_query .= " AND second_attendance.subject_name = '$search_subject'";
}
if ($search_date) {
    $base_query .= " AND second_attendance.date = '$search_date'";
}
if ($search_status) {
    $base_query .= " AND second_attendance.status = '$search_status'";
}
if ($search_week) {
    $base_query .= " AND WEEK(second_attendance.date, 1) = WEEK('$search_week', 1) AND YEAR(second_attendance.date) = YEAR('$search_week')";
}
if ($search_month && $search_year) {
    $base_query .= " AND MONTH(second_attendance.date) = '$search_month' AND YEAR(second_attendance.date) = '$search_year'";
}

$second_attendance_query = "SELECT second_students.name AS second_student_name, second_students.roll_number, 
                            second_attendance.subject_name, second_attendance.status, second_attendance.date 
                     $base_query 
                     ORDER BY second_attendance.date DESC, second_students.name ASC
                     LIMIT $limit OFFSET $offset";

$second_attendance_result = $conn->query($second_attendance_query);
if (!$second_attendance_result) {
        ...
        <div class="card-body">
            <form method="GET" class="row g-3">
                ...
                <div class="col-md-6 col-lg-3 d-flex align-items-end">
                    ...
                    <a href="2nd_principal_view_attendance.php" class="btn btn-outline-secondary">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-hover">
                    ...
                    <tbody>
                        <?php if ($second_attendance_result->num_rows > 0): ?>
                            <?php while ($row = $second_attendance_result->fetch_assoc()): 
                                $date = new DateTime($row['date']);
                                $dayName = $date->format('l');
                                $formattedDate = $date->format('M j, Y');
                            ?>
                                <tr>
                                    <td><?= htmlspecialchars($row['second_student_name']); ?></td>
